...
async def analyze_token(data: dict) -> tuple[bool, str]:
    ...
    # ❌ FILTER 3: Kein Social Media (schlechtes Zeichen)
    has_social = bool(twitter or telegram or website)
    if not has_social:
        return False, "Kein Social Media"

    # ❌ FILTER 4: Top 10 Halter haben zu viel (Manipulation)
    ...
    # ✅ GUTES SIGNAL: Social Media vorhanden
    social_count = sum([bool(twitter), bool(telegram), bool(website)])
    if social_count >= 2:
        score += 2
        reasons.append(f"✅ {social_count} Social Media Links")
    else:
        score += 1
        reasons.append(f"⚠️ {social_count} Social Media Link")
